    ...
    return (
        ...
        >
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-extrabold tracking-tight">Section Management</h1>
                    <p className="text-muted-foreground mt-1">
                        Manage all individual test sections across all mock tests.
                    </p>
                </div>
                <Button onClick={() => setCreateOpen(true)} className="rounded-xl gap-2 shadow-lg shadow-primary/20">
            ...
            {isLoading ? (
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    {sections.map((section: any) => (
                        <Card key={section._id} className="group rounded-3xl border-slate-200/60 hover:border-primary/30 transition-all hover:shadow-xl hover:shadow-primary/5 bg-white overflow-hidden">
                            <CardContent className="p-0">
                                <div className="p-5 flex justify-between items-start">
                                    <div className="space-y-3 flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            <div className="p-2 bg-primary/5 rounded-xl text-primary">
                                                {SECTION_ICONS[section.name] || <FileQuestion className="h-4 w-4" />}
                                            </div>
                                            <Badge variant="outline" className="capitalize text-[10px] h-5 px-1.5 font-bold border-primary/20 text-primary">
                                                {section.name}
                                            </Badge>
                                        </div>
                                        <div>
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end" className="rounded-xl w-40">
                                            <DropdownMenuItem onClick={() => { setEditSection(section); setEditOpen(true); }} className="gap-2">
                                                <Pencil className="h-4 w-4" /> Edit Settings
                                            </DropdownMenuItem>
                                            <DropdownMenuSeparator />
                                            <DropdownMenuItem
                                                onClick={() => handleDelete(section._id, section.mockTest?._id || section.mockTest)}
                                ...
                                <div className="px-5 py-4 bg-slate-50 flex items-center justify-between border-t border-slate-100">
                                    <div className="flex gap-4">
                                        <div className="flex flex-col">
                                            <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Questions</span>
                                            <span className="text-sm font-bold">{section.questions?.length || 0}</span>
                                        </div>
                                        <div className="flex flex-col">
                                    ...
                                    <Link to={`/dashboard/mock-test-sections/${section._id}`}>
                                        <Button size="sm" className="rounded-xl gap-2 font-bold px-4 hover-lift">
