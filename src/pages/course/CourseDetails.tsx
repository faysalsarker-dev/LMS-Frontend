...
const ListItem = memo(({ text, index, color = "primary" }: { text: string; index: number; color?: string }) => (
  <motion.li 
    initial={{ opacity: 0, x: -20 }}
    animate={{ opacity: 1, x: 0 }}
    transition={{ delay: index * 0.1 }}
    className="flex items-start gap-3 text-muted-foreground"
  >
    <div className={`w-1.5 h-1.5 bg-${color} rounded-full mt-2.5 flex-shrink-0`} />
    {text}
  </motion.li>
));

const MilestoneCard = memo(({ milestone, index }: { milestone: IMilestone; index: number }) => (
  <motion.div
    initial={{ opacity: 0, y: 20 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ delay: index * 0.1 }}
...
const CourseFeatureItem = memo(({ icon: Icon, text, color, index }: any) => (
  <motion.div
    initial={{ opacity: 0, x: -20 }}
    animate={{ opacity: 1, x: 0 }}
    transition={{ delay: 0.5 + index * 0.1 }}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const PriceDisplay = memo(({ course, finalPrice }: any) => (
  <div className="flex items-center gap-3">
    {course.isDiscounted ? (
      <>
        <span className="text-3xl font-bold text-primary">{course.currency} {finalPrice}</span>
...
const CourseDetails = () => {
  ...
  const finalPrice = useMemo(() => 
    course?.isDiscounted ? course?.discountPrice : course?.price,
    [course?.isDiscounted, course?.discountPrice, course?.price]
  );

  const isInWishlist = useMemo(() => 
    !isUserLoading && user?.wishlist?.includes(course?._id),
    [isUserLoading, user?.wishlist, course?._id]
  );

  const isEnrolled = useMemo(() => 
    !isUserLoading && user?.courses?.includes(course?._id),
    [isUserLoading, user?.courses, course?._id]
  );
  ...
  const courseFeatures = useMemo(() => [
    ...
    { icon: CheckCircle, text: "Full lifetime access", color: "text-violet-500" },
    { icon: CheckCircle, text: "Certificate of completion", color: "text-amber-500" }
  ], [course?.duration, course?.totalLectures]);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        ...
        >
          <div className="container mx-auto p-6 bg-gradient-primary">
            <motion.div variants={ANIMATION_VARIANTS.hero}>
              <Card className="overflow-hidden py-0 border-boder bg-gradient-card backdrop-blur-glass border-glass shadow-glass">
                <CardContent className="p-0">
                  <div className="grid lg:grid-cols-2 gap-0">
                    {/* Left Content */}
                      ...
                      <motion.div variants={ANIMATION_VARIANTS.item} className="space-y-6" >
                        ...
                        <div className="flex flex-wrap items-center gap-6 pt-4">
                          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                            <Link to={isEnrolled ? `/course/video/${course._id}` : `/checkout/${course.slug}`}>
                              <Button size="lg" className="bg-gradient-primary text-primary-foreground px-8 py-3 text-lg font-semibold shadow-elevated">
                          ...
                          <PriceDisplay course={course} finalPrice={finalPrice} />
                        </div>
                      </motion.div>
                    </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              ...
              >
                <Tabs defaultValue="overview" className="w-full">
                  <motion.div variants={ANIMATION_VARIANTS.item}>
                    <TabsList className="grid w-full grid-cols-3 mb-4 border-glass mt-10 h-12">
                      <TabsTrigger value="overview" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground p-3 font-semibold transition-all">
                        Overview
                      </TabsTrigger>
                      <TabsTrigger value="curriculum" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground p-3 font-semibold transition-all">
                        Curriculum
                      </TabsTrigger>
                      <TabsTrigger value="reviews" className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground p-3 font-semibold transition-all">
                        Reviews
                      </TabsTrigger>
                    </TabsList>
                  </motion.div>
                    ...
                    <motion.div variants={ANIMATION_VARIANTS.item}>
                      <Card className="bg-gradient-card backdrop-blur-glass border-glass shadow-soft hover:shadow-elevated transition-shadow">
                        ...
                        <CardContent>
                          <ul className="space-y-3">
                            {course.requirements?.map((req: string, index: number) => (
                              <ListItem key={index} text={req} index={index} />
                            ))}
                          </ul>
                        </CardContent>
                      </Card>
                    ...
                    <motion.div variants={ANIMATION_VARIANTS.item}>
                      <Card className="bg-gradient-card backdrop-blur-glass border-glass shadow-soft hover:shadow-elevated transition-shadow">
                        ...
                        <CardContent>
                          <ul className="space-y-3">
                            {course.prerequisites?.map((pre: string, index: number) => (
                              <ListItem key={index} text={pre} index={index} color="amber-500" />
                            ))}
                          </ul>
                        </CardContent>
                      </Card>
                    ...
                    <motion.div variants={ANIMATION_VARIANTS.item}>
                      <Card className="bg-gradient-card backdrop-blur-glass border-glass shadow-soft">
                        ...
                        <CardContent>
                          <div className="space-y-4">
                            {course.milestones.map((ml: IMilestone, index: number) => (
                              <MilestoneCard key={ml._id} milestone={ml} index={index} />
                            ))}
                          </div>
                        </CardContent>
                      </Card>
              ...
              <motion.div 
                ...
                animate="visible"
              >
                <motion.div variants={ANIMATION_VARIANTS.item} className="sticky top-24">
                  <Card className="bg-gradient-card backdrop-blur-glass border-glass shadow-glass overflow-hidden p-0">
                    <CardContent className="p-0">
                      <div className="relative aspect-video">
                        <img
                      ...
                      <div className="p-6 space-y-6">
                        ...
                        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                          <Link to={isEnrolled ? `/course/video/${course._id}` : `/checkout/${course.slug}`} className="block">
                            <Button size="lg" className="w-full bg-gradient-primary text-primary-foreground px-8 py-3 text-lg font-semibold shadow-elevated">
                              {isEnrolled ? "Continue Learning" : "Enroll Now"}
                            </Button>
                          </Link>
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-3">
                            {courseFeatures.map((item, index) => (
                              <CourseFeatureItem key={index} {...item} index={index} />
                            ))}
                          </div>
                        </div>
                      </div>
