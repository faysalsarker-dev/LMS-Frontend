...
import type { IListeningResult } from "@/utils/mock-test/listening.utils";

interface Props {
  results: IListeningResult;
  questions: IMockQuestion[];
  slug: string;
}

export const ListeningResult = ({ results, questions, slug }: Props) => {
  ...
  return (
    <div className="min-h-[80vh] container max-w-4xl mx-auto py-12 space-y-8">
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground">Your performance in the Listening section.</p>
      </div>
          ...
          {results.items.map((item, idx) => (
            <div key={item.questionId} className={`p-6 rounded-2xl border-2 ${item.isCorrect ? 'bg-green-500/5 border-green-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
              <div className="flex items-start justify-between gap-4">
                ...
                <Badge variant="outline" className="font-bold">{item.isCorrect ? item.marks : 0} / {item.marks} pts</Badge>
              </div>
            </div>
          ))}
        </div>
