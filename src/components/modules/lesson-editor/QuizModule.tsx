...
const createDefaultQuestion = (type: QuestionType): IQuestion => {
  const base = {
    ...
    timer: undefined,
    options: [],
    correctAnswer: '', // Will store the correct answer text/value
    modelAnswer: '',
    audioFile: null,
    audioUrl: '',
  };
  ...
  switch (type) {
    ...
    case 'true_false':
      return {
        ...base,
        options: [
          { text: 'True' }, // Removed isCorrect
          { text: 'False' }, // Removed isCorrect
        ],
      };
    default:
      return base;
...
export function QuizModule({ questions, onQuestionsChange }: QuizModuleProps) {
  ...
  const addOption = (questionIndex: number) => {
    const updated = [...questions];
    updated[questionIndex].options.push({ text: '' }); // Removed isCorrect
    onQuestionsChange(updated);
  };

  const removeOption = (questionIndex: number, optionIndex: number) => {
    const updated = [...questions];
    const removedOptionText = updated[questionIndex].options[optionIndex].text;
    
    // If removing the correct answer, clear correctAnswer field
    if (updated[questionIndex].correctAnswer === removedOptionText) {
  ...
  const updateOption = (
    ...
    if (field === 'isCorrect' && value === true) {
      // Set this option's text as the correctAnswer
      const optionText = updated[questionIndex].options[optionIndex].text;
      updated[questionIndex].correctAnswer = optionText;
    } else if (field === 'text') {
      const oldText = updated[questionIndex].options[optionIndex].text;
      ...
      // If this was the correct answer, update correctAnswer to new text
      if (updated[questionIndex].correctAnswer === oldText) {
        updated[questionIndex].correctAnswer = value as string;
      }
    }
